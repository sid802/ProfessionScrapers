...
class HebCivilEngineerProfessor(object):
...
def parse_professor_from_page(p_id, page_source, page_url, domain):
	...
	if name_title != '':
		...
		professor = HebCivilEngineerProfessor(p_id = p_id, name=name_title, job=job, phone=phone, fax=fax, email=email, personal_site=personal_site, room=room, research_interests=research_interests, pic_url=pic_url, source=page_url)
		return professor
	return None
...
def main():
	BASE_URL=r'http://cee.technion.ac.il/CivilEng/Templates/ShowPage.asp?DBID=1&TMID=139&LNGID=2&FID=166&PID=0&IID={id}'
