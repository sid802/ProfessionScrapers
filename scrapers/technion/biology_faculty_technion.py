...
class BiologyFaculty(object):
	attributes_ordered = ['name', 'job', 'phone', 'fax', 'email', 'interests', 'research', 'office_location', 'pic_url']
	def __init__(self, name='', job='', phone='', fax='', email='', interests='', research='', office_location='', pic_url='', specialties=''):
		self.name=name
		self.job=job
		self.phone=phone
		self.fax=fax
...
def parse_professor_from_row(professor_row_element, url_domain=''):
	...
	fax_path = './/span[@class="fax"]/text()'
	email_path = './/div[@class="memberEmail"]/a/text()'
	interests_path = './/div[@class="memberInterests"]/text()'
	research_path = './/div[@class="memberInterests"]/a/text()'
	...
	if name != '':
		...
		fax = first_item_or_null(professor_row_element.xpath(fax_path))
		email = first_item_or_null(professor_row_element.xpath(email_path))
		interests = clean_and_concat(professor_row_element.xpath(interests_path))
		research = clean_and_concat(professor_row_element.xpath(research_path))
		...
		professor = BiologyFaculty(name=name, job=job, phone=phone, fax=fax, email=email, interests=interests, research=research, office_location=office_location, pic_url=pic_url)
		return professor
	return None
